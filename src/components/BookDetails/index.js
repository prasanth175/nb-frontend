...
class BookDetails extends Component {
  ...
  getBookDetails = async () => {
    ...
    const {params} = match
    const {id} = params
    const bookDetailsUrl = `https://novelty-bookstore.vercel.app/books/${id}`
  ...
  getBiddingDetails = async () => {
    ...
    const url = 'https://novelty-bookstore.vercel.app/book-bid-details'
    const options = {
      headers: {
  ...
  submitBidForm = async event => {
    ...
    const {bookId, description, title, file} = bookDetails
    const bidDetails = {
      bookId,
      bidAmount,
      mobile: mobileNumber,
    ...
    const bidUrl = 'https://novelty-bookstore.vercel.app/biddetails'
    const options = {
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${getToken}`,
