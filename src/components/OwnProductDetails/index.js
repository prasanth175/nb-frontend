...
class OwnProductDetails extends Component {
  ...
  getOwnBookDetails = async () => {
    ...
    const {params} = match
    const {id} = params
    const bookDetailsUrl = `https://novelty-bookstore.vercel.app/products/${id}`
  ...
  getBidDetails = async () => {
    ...
    const bidDetailsUrl = 'https://novelty-bookstore.vercel.app/biddata'
    const options = {
      headers: {
  ...
  onDeleteOwnBook = async () => {
    ...
    const {params} = match
    const {id} = params
    const delUrl = `https://novelty-bookstore.vercel.app/products/${id}`
    const options = {
      method: 'DELETE',
    }
